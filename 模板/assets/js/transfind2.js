...
$(function(){
	$("#transfind2_inquire").ajaxForm(function(response_text)
	{
	//	alert(response_text);
		refresh_main_table();
	...
	//		alert(response_text);
		...
		transfind2_show_total();
		transfind2_write_table(main_json[0].total_num);
	 });
});
...
function transfind2_show_total()
{
	document.getElementById('transfind2_total').innerHTML = '总共查询到<b>' + main_json[0].total_num +'</b>条记录';
}
var tab =document.getElementById("transfind2_main_table");
var tr = new Array();
tr[1] = 'abc';
function transfind2_write_table(m)
{
	for(var i = 1; i <= m; i++)
	{
		...
		// details_button.setAttribute("class", "btn btn-link transfind2_show_details_button");
		...
		details.innerHTML = '<button class="btn btn-link" style="font-size:12px" onclick=transfind2_show_details(' + i + ');>详情</button>';
...
function transfind2_show_details(x)
{
