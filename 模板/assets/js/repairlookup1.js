...
$(function(){
	$("#repairlookup1_inquire").ajaxForm(function(response_text)
	{
		// alert(response_text);
		refresh_main_table();
		...
		repairlookup1_show_total();
		repairlookup1_write_table(main_json[0].total_num);
	 });
});
...
function repairlookup1_show_total()
{
//	alert("dzsdfaef");
	document.getElementById("repairlookup1_total").innerHTML = "总共查询到<b>" +main_json[0].total_num + "</b>条记录";
}

var tab =document.getElementById("repairlookup1_main_table");
var tr = new Array();
tr[1] = 'abc';
function repairlookup1_write_table(m)
{
	for(var i = 1; i <= m; i++)
	{
		...
		// details_button.setAttribute("class", "btn btn-link repairlookup1_show_details_button");
		...
		details.innerHTML = '<button class="btn btn-link" style="font-size:12px" onclick=repairlookup1_show_details(' + i + ');>详情</button>';
...
function repairlookup1_show_details(x)
{
